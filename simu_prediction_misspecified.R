...
yGenerate <- function(x, beta, SNR, noise_type) {
  ...
  for (spar_type in c("R1", "R2", "R3", "R4", "R5")) {
    ...
    for(i in 1:nloop){
      ...
      for(j in 1:length(sub_meta)){
        ...
        ######################################################
        coef <- cbind(ols_temp, unif_temp, blev_temp, slev_temp, iboss_temp, core_temp)
        coef[is.na(coef)] <- 0
...
pdf("pmse_misspecified.pdf", width = 17.5, height = 10.5)
