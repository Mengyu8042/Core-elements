...
if (is_corrupt == TRUE) {
  file_name <- "pmse_corrupt.pdf"
} else {
  file_name <- "pmse_uncorrupt.pdf"
}
  ...
  for (spar_type in c("R1", "R2", "R3", "R4", "R5")) {
    ...
    for (i in 1:nloop) {
      ...
      if (is_corrupt == TRUE) {
        ...
        ind_shuffle <- sample(N_train, N_train, replace = FALSE)
        X <- X[ind_shuffle, ]
        Y <- Y[ind_shuffle]
      }
      ...
      D <- svdf$d
      V <- svdf$v
      ols_temp <- as.vector(V %*% diag(1/D) %*% t(U) %*% Y)
      ...
      for (j in 1:length(sub_meta)) {
        ...
        ## Subsampling methods ## 
        # UNIF
        id <- sample(N_train, sub, replace = FALSE)
        xt <- X[id, ]
        yt <- Y[id]
        fit <- lm(yt ~ xt - 1)
        ...
        # BLEV
        id <- sample(N_train, sub, replace = FALSE, prob_blev)
        xt <- X[id, ]
        yt <- Y[id]
        wgt <- 1/prob_blev[id]
        ...
        # SLEV
        id <- sample(N_train, sub, replace = FALSE, prob_slev)
        xt <- X[id, ]
        yt <- Y[id]
        wgt <- 1/prob_slev[id]
        fit <- lm(yt ~ xt - 1, weights = wgt)
        ...
        for (k in 1:K) {
          ...
          D <- svdf$d
          V <- svdf$v
          ols_dc_temp[k, ] <- as.vector(V %*% diag(1/D) %*% t(U) %*% Y_k)
        ...
        ######################################################
        coef <- cbind(ols_temp, ols_dc, unif_temp, blev_temp, slev_temp,
                      iboss_temp, core_temp, core_dc)
        coef[is.na(coef)] <- 0
    ...
    ## Plot the results ##
    if (is_corrupt == TRUE) {
      mse_mat <- data.frame(mse = c(apply(OLS, 2, mean), apply(OLS_MOM, 2, mean), apply(UNIF, 2, mean), apply(BLEV, 2, mean), 
                                    apply(SLEV, 2, mean), apply(IBOSS, 2, mean), apply(CORE, 2, mean), apply(CORE_MOM, 2, mean)), 
                            sd = c(apply(OLS, 2, sd), apply(OLS_MOM, 2, sd), apply(UNIF, 2, sd), apply(BLEV, 2, sd), 
                                   apply(SLEV, 2, sd), apply(IBOSS, 2, sd), apply(CORE, 2, sd), apply(CORE_MOM, 2, sd)), 
                            Method = factor(rep(c("FullOLS", "MOM-OLS", "UNIF", "BLEV", "SLEV", 
                                                  "IBOSS", "CORE", "CORE-MOM"), each = length(sub_meta))),
      ...
      p23 <- p1 + theme_bw() + theme(panel.grid.major = element_blank(),
        ...
        scale_shape_manual(values = c(9, 3, 8, 4, 5, 7, 1, 2)) + 
        scale_linetype_manual(values = c(6, 3, 5, 4, 3, 2, 1, 1)) + 
        scale_size_manual(values = c(1, 1, 1, 1, 1, 1, 1, 1)) + 
        scale_color_manual(values = c("grey2", "black", "#999999", "#0033CC", 
                                      "#3399FF", "orchid", "red", "#FF9900")) + 
        labs(title = paste(case, collapse = ", "), x = "r/p", y = "log(PMSE)") + 
      ...
      mse_mat <- data.frame(mse = c(apply(OLS, 2, mean), apply(UNIF, 2, mean), apply(BLEV, 2, mean), 
                                    apply(SLEV, 2, mean), apply(IBOSS, 2, mean), apply(CORE, 2, mean)), 
                            sd = c(apply(OLS, 2, sd), apply(UNIF, 2, sd), apply(BLEV, 2, sd), 
                                   apply(SLEV, 2, sd), apply(IBOSS, 2, sd), apply(CORE, 2, sd)), 
                            Method = factor(rep(c("FullOLS", "UNIF", "BLEV", "SLEV", "IBOSS", "CORE"), 
                                                each = length(sub_meta))),
                            sub = rep(sub_meta/p, num_method - 2))
      ...
      p23 <- p1 + theme_bw() + theme(panel.grid.major = element_blank(),
        ...
        geom_line(aes(linetype = Method, size = Method), position = pd) + 
        geom_point(position = pd, aes(shape = Method), size = 2) + 
        scale_shape_manual(values = c(9, 8, 4, 5, 7, 1)) + 
        scale_linetype_manual(values = c(6, 5, 4, 3, 2, 1)) + 
        scale_size_manual(values = c(1, 1, 1, 1, 1, 1)) + 
        scale_color_manual(values = c("grey2", "#999999", "#0033CC", "#3399FF", "orchid", "red")) + 
        labs(title = paste(case, collapse = ", "), x = "r/p", y = "log(PMSE)") + 
        theme(plot.title = element_text(hjust = 0.5, size = 19))
    }
