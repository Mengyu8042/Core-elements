...
## Plot the results ##
UNIF <- log10(mse_meta[, 1, ])
BLEV <- log10(mse_meta[, 2, ])
SLEV <- log10(mse_meta[, 3, ])
IBOSS <- log10(mse_meta[, 4, ])
CORE <- log10(mse_meta[, 5, ])
CORE_MOM <- log10(mse_meta[, 6, ])
...
p23
ggsave(filename = "example2_mse.png", width = 6, height = 3.5, units = "in", dpi = 300)
